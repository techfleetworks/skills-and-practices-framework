...
Source for the framework's user guide, published at [`/learn`](../learn). It's a small
...
and commit the **static output** to `../learn`. Nothing is loaded from a CDN at runtime —
...
repo's `data/` folder, or open the built `../learn` output.

## Build (regenerates ../learn)
```bash
npm install
npm run build
```
Output goes to `../learn` (`index.html` + one hashed JS bundle). Commit those alongside any
